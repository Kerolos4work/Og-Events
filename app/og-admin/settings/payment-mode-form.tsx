...
export default function PaymentModeForm() {
  ...
  const [state, formAction] = useActionState(updatePaymentMode, { message: '' });
  ...
  // Handle form submission to update the UI state
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsPending(true);
    
    const formData = new FormData(e.currentTarget);
    const selectedMode = formData.get('payment-mode') as string;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" disabled={state.pending}>
        {state.pending ? 'Updating...' : 'Update Payment Mode'}
      </Button>
    </form>
  );
}
